...
export async function verifyFullPageScrolling(browser, script) {
  ...
  try {
    ...
    for (let iteration = 0; iteration < 8; iteration += 1) {
      ...
      assert.deepEqual(result.positions, [0, 640, 1280, 1760].map((scrollY) => ({ scrollX: 0, scrollY })), 'Capture scrolling must reach each target despite smooth!important');
